#include "pkgs.h"
/*
 * @lc app=leetcode.cn id=339 lang=cpp
 *
 * [339] 嵌套列表加权和
 *
 * See 364. 嵌套列表加权和 II:
 ...
 *
 * 给定一个嵌套的整数列表 nestedList ，每个元素要么是整数，要么是列表。
 * 同时，列表中元素同样也可以是整数或者是另一个列表。
 *
 * 整数的 深度 是其在列表内部的嵌套层数。
 * 例如，嵌套列表 [1,[2,2],[[3],2],1] 中每个整数的值就是其深度。
 *
 * 请返回该列表按深度加权后所有整数的总和。
 *
 * 示例 1：
...
public:
    int depthSum(vector<NestedInteger> &nestedList)
    {
        function<int(vector<NestedInteger> &, int)> dfs =
            [&](vector<NestedInteger> &nestedList, int level) -> int
        {
            int res = 0;

            for (auto node : nestedList)
            {
                res += node.isInteger() ? level * node.getInteger()
