...
private:
    queue<int> q;
    mutex mtx;
    counting_semaphore<INT_MAX> spaceAvailable; // 控制可用空间
    counting_semaphore<INT_MAX> itemsAvailable; // 控制可取元素数

public:
    BoundedBlockingQueue(int capacity)
        : spaceAvailable(capacity), itemsAvailable(0) {} // space初始化为capacity, items为0

    void enqueue(int element)
    {
        spaceAvailable.acquire(); // 等待可用空间（P 操作）
        ...
        itemsAvailable.release(); // 增加可用元素数（V 操作）
    }

    int dequeue()
    {
        itemsAvailable.acquire(); // 等待队列中有元素（P 操作）
        ...
        spaceAvailable.release(); // 增加可用空间数（V 操作）
